...
# Extract all completed datasets from mids object
imp_list <- complete(imputed_datasets_wide, action = "all")

# Function to get pooled descriptive stats for one variable
pool_descriptives <- function(varname, imp_list) {
  
  # Mean in each imputed dataset
  means <- sapply(imp_list, function(dat) mean(dat[[varname]], na.rm = TRUE))
  
  # Variance in each imputed dataset
  vars  <- sapply(imp_list, function(dat) var(dat[[varname]], na.rm = TRUE))
  
  # Pooled mean
  pooled_mean <- mean(means)
  
  # Pooled variance of the variable itself
  # law of total variance: average within-imputation variance + between-imputation variance of means
  pooled_var <- mean(vars) + var(means)
...
pooled_stats <- map_dfr(cbcl_vars, ~ pool_descriptives(.x, imp_list))

# Create table
table_S5_imp <- data.frame(
  Outcome = outcomes,
  Age = timepoints,
  Raw_Mean = round(pooled_stats$Raw_Mean, 2),
  Raw_SD = round(pooled_stats$Raw_SD, 2),
  stringsAsFactors = FALSE
)

# sqrt(mean)
table_S5_imp$Sqrt_Mean <- round(sqrt(table_S5_imp$Raw_Mean), 2)

# Back-transform to raw scale for +0.5 and +1.0 sqrt units
table_S5_imp$Raw_at_plus_0.5 <- round((table_S5_imp$Sqrt_Mean + 0.5)^2, 2)
table_S5_imp$Delta_Raw_0.5 <- round(table_S5_imp$Raw_at_plus_0.5 - table_S5_imp$Raw_Mean, 2)

table_S5_imp$Raw_at_plus_1.0 <- round((table_S5_imp$Sqrt_Mean + 1.0)^2, 2)
table_S5_imp$Delta_Raw_1.0 <- round(table_S5_imp$Raw_at_plus_1.0 - table_S5_imp$Raw_Mean, 2)

# Combined Mean (SD)
table_S5_imp$Raw_Mean_SD <- paste0(table_S5_imp$Raw_Mean, " (", table_S5_imp$Raw_SD, ")")

# Final table
final_table_imp <- table_S5_imp %>%
  mutate(`Outcome (Domain)` = Outcome) %>%
  select(
    `Outcome (Domain)`,
    `Age (Years)` = Age,
    `Mean (SD)` = Raw_Mean_SD,
...
# Reorder rows
desired_order <- c("Internalizing", "Externalizing", "ADHD")
ages_order <- c("6", "11", "15")

final_table_imp <- final_table_imp %>%
  arrange(
    factor(`Outcome (Domain)`, levels = desired_order),
    factor(`Age (Years)`, levels = ages_order)
  )
...
# Detect the percentage of missing values
missvalues1 <- cbind("# NA" = sort(colSums(is.na(d1))),
                     "% NA" = round(sort(colMeans(is.na(d1))) * 100, 2))
...
predictors_to_use <- c(
  ...
  "urine_z_metabolite_score_int6",
  "urine_z_metabolite_score_adhd6",
  "mother_age_centered", "z_who_bmi_7y", "sex", "passive_smk_6y", "mum_educ", "m_greek", "age6")
...
ls()
#ddd <- completed_list_updated[[1]]
#Convert from wide format to long format the imputed datasets
wide_to_long <- function(wideformat) {
...
long_list <- lapply(completed_list_updated, wide_to_long)
...
################################################################################
#Create the new variables of the cuttoff scores with the square root transformed values 
#We rounded in 4 decimal places the cuttoff score as our scores are
load("imputed_datasets_wide.RData")

completed_list_updated <- lapply(completed_list, function(d) {
  d %>%
    mutate(
      internalizing_6_sqrt_bin = case_when(
        sex == "Female" & round(sqrt_cbcl_pro_int6, 4) <  round(sqrt(11), 4) ~ "Normal",
        sex == "Female" & round(sqrt_cbcl_pro_int6, 4) >= round(sqrt(11), 4) ~ "Borderline/Clinical",
        sex == "Male"   & round(sqrt_cbcl_pro_int6, 4) <  round(sqrt(9), 4)  ~ "Normal",
        sex == "Male"   & round(sqrt_cbcl_pro_int6, 4) >= round(sqrt(9), 4)  ~ "Borderline/Clinical",
        TRUE ~ NA_character_
      ),
      
      internalizing_11_sqrt_bin = case_when(
        sex == "Female" & round(sqrt_cbcl_int11, 4) <  round(sqrt(11), 4) ~ "Normal",
        sex == "Female" & round(sqrt_cbcl_int11, 4) >= round(sqrt(11), 4) ~ "Borderline/Clinical",
        sex == "Male"   & round(sqrt_cbcl_int11, 4) <  round(sqrt(9), 4)  ~ "Normal",
        sex == "Male"   & round(sqrt_cbcl_int11, 4) >= round(sqrt(9), 4)  ~ "Borderline/Clinical",
        TRUE ~ NA_character_
      ),
      
      internalizing_15_sqrt_bin = case_when(
        sex == "Female" & round(sqrt_cbcl_int15, 4) <  round(sqrt(12), 4) ~ "Normal",
        sex == "Female" & round(sqrt_cbcl_int15, 4) >= round(sqrt(12), 4) ~ "Borderline/Clinical",
        sex == "Male"   & round(sqrt_cbcl_int15, 4) <  round(sqrt(11), 4) ~ "Normal",
        sex == "Male"   & round(sqrt_cbcl_int15, 4) >= round(sqrt(11), 4) ~ "Borderline/Clinical",
        TRUE ~ NA_character_
      ),
      
      externalizing_6_sqrt_bin = case_when(
        sex == "Female" & round(sqrt_cbcl_ext6, 4) <  round(sqrt(12), 4) ~ "Normal",
        sex == "Female" & round(sqrt_cbcl_ext6, 4) >= round(sqrt(12), 4) ~ "Borderline/Clinical",
        sex == "Male"   & round(sqrt_cbcl_ext6, 4) <  round(sqrt(12), 4) ~ "Normal",
        sex == "Male"   & round(sqrt_cbcl_ext6, 4) >= round(sqrt(12), 4) ~ "Borderline/Clinical",
        TRUE ~ NA_character_
      ),
      
      externalizing_11_sqrt_bin = case_when(
        sex == "Female" & round(sqrt_cbcl_ext11, 4) <  round(sqrt(12), 4) ~ "Normal",
        sex == "Female" & round(sqrt_cbcl_ext11, 4) >= round(sqrt(12), 4) ~ "Borderline/Clinical",
        sex == "Male"   & round(sqrt_cbcl_ext11, 4) <  round(sqrt(12), 4) ~ "Normal",
        sex == "Male"   & round(sqrt_cbcl_ext11, 4) >= round(sqrt(12), 4) ~ "Borderline/Clinical",
        TRUE ~ NA_character_
      ),
      
      externalizing_15_sqrt_bin = case_when(
        sex == "Female" & round(sqrt_cbcl_ext15, 4) <  round(sqrt(12), 4) ~ "Normal",
        sex == "Female" & round(sqrt_cbcl_ext15, 4) >= round(sqrt(12), 4) ~ "Borderline/Clinical",
        sex == "Male"   & round(sqrt_cbcl_ext15, 4) <  round(sqrt(14), 4) ~ "Normal",
        sex == "Male"   & round(sqrt_cbcl_ext15, 4) >= round(sqrt(14), 4) ~ "Borderline/Clinical",
        TRUE ~ NA_character_
      )
    ) %>%
    mutate(
      internalizing_6_sqrt_bin  = factor(internalizing_6_sqrt_bin,  levels = c("Normal", "Borderline/Clinical")),
      internalizing_11_sqrt_bin = factor(internalizing_11_sqrt_bin, levels = c("Normal", "Borderline/Clinical")),
      internalizing_15_sqrt_bin = factor(internalizing_15_sqrt_bin, levels = c("Normal", "Borderline/Clinical")),
      externalizing_6_sqrt_bin  = factor(externalizing_6_sqrt_bin,  levels = c("Normal", "Borderline/Clinical")),
      externalizing_11_sqrt_bin = factor(externalizing_11_sqrt_bin, levels = c("Normal", "Borderline/Clinical")),
      externalizing_15_sqrt_bin = factor(externalizing_15_sqrt_bin, levels = c("Normal", "Borderline/Clinical"))
    )
})

#Save the new dataset 
#save(completed_list_updated, file = "imputed_datasets_wide.RData")
View(completed_list_updated[[1]])

#Check if there are differences in the cutt off scores with the sqrt transformed values comparing with the raw scores 
#Repite for all the outcomes and the ages
df <- completed_list_updated[[1]]

mismatch_cases <- subset(
  df,
  Diagnosis_internalizing_6_combined == "Borderline/Clinical" &
    internalizing_6_sqrt_bin == "Normal"
)
...
unique(mi_results$n_subjects)
unique(cc_results$n_subjects)
...
completed_list <- lapply(completed_list_updated, function(d) {
  d %>%
    mutate(
      sex = factor(sex, levels = c("Male", "Female")),
      m_greek = factor(m_greek, levels = c("Greek", "Other")),
...
sapply(
  completed_list_updated[[1]][, c("sex", "m_greek", "mum_educ", "passive_smk_6y", "bmi_cutoff_merge")],
  class
)

wide_list <- completed_list_updated
age_outcomes <- list(
...
plot_data <- mi_age_specific_results %>%
  ...
  mutate(
    ...
    Outcome = case_when(
      ...
      grepl("_adhd$", model) ~ "ADHD"
    ),
    Source = case_when(
      grepl("^serum", model) ~ "Serum",
      grepl("^urine", model) ~ "Urine"
    ),
    Outcome = factor(Outcome, levels = c("Internalizing", "Externalizing", "ADHD")),
...
ggplot(plot_data, aes(x = Age, y = Beta, group = 1)) +
  ...
  labs(
    title = "Associations Between Metabolite Scores and Symptoms by Age, Outcome, and Biospecimen",
    x = "Age at Assessment",
...
#Binary analysis with cutoff scores

load("imputed_datasets_wide.RData")

imp_sqrt_binary <- miceadds::datalist2mids(completed_list_updated)

completed_list_sqrt_binary <- lapply(completed_list_updated, function(d) {
  d %>%
    mutate(
      ...
      ## Square-root binary outcomes as factors
      internalizing_6_sqrt_bin = factor(
        internalizing_6_sqrt_bin,
        levels = c("Normal", "Borderline/Clinical")
      ),
      internalizing_11_sqrt_bin = factor(
        internalizing_11_sqrt_bin,
        levels = c("Normal", "Borderline/Clinical")
      ),
      internalizing_15_sqrt_bin = factor(
        internalizing_15_sqrt_bin,
        levels = c("Normal", "Borderline/Clinical")
      ),
      
      externalizing_6_sqrt_bin = factor(
        externalizing_6_sqrt_bin,
        levels = c("Normal", "Borderline/Clinical")
      ),
      externalizing_11_sqrt_bin = factor(
        externalizing_11_sqrt_bin,
        levels = c("Normal", "Borderline/Clinical")
      ),
      externalizing_15_sqrt_bin = factor(
        externalizing_15_sqrt_bin,
        levels = c("Normal", "Borderline/Clinical")
      )
    )
})
...
diagnosis_specs_sqrt <- list(
  serum_ext = list(
    outcome_base = "externalizing",
...
pooled_results_list <- lapply(names(diagnosis_specs_sqrt), function(name) {
  
  spec <- diagnosis_specs_sqrt[[name]]
  
  lapply(ages, function(age_val) {
    
    outcome_var <- paste0(spec$outcome_base, "_", age_val, "_sqrt_bin")
    ...
    summary(pooled, conf.int = TRUE, exponentiate = TRUE) %>%
      mutate(
        model_type = "sqrt_binary_logistic_model",
        model = name,
        age = age_val,
        outcome = outcome_var,
...
logistic_results_sqrt <- bind_rows(pooled_results_list)

# Fix CI column names if needed
logistic_results_sqrt <- logistic_results_sqrt %>%
  rename_with(~ "conf.low", matches("^2\\.5 %$")) %>%
  rename_with(~ "conf.high", matches("^97\\.5 %$"))
...
logistic_results_sqrt

#Calculate the count number of normal and borderline/clinical and the percentages

ages <- c(6, 11, 15)
symptoms <- c("internalizing", "externalizing")

count_results <- lapply(seq_along(completed_list_updated), function(i) {
  dat <- completed_list_updated[[i]]
  
  lapply(symptoms, function(symptom) {
    lapply(ages, function(age) {
      varname <- paste0(symptom, "_", age, "_sqrt_bin")
      ...
      data.frame(
        ...
        symptom = symptom,
        age = age,
        Normal = sum(x == "Normal", na.rm = TRUE),
        Borderline_Clinical = sum(x == "Borderline/Clinical", na.rm = TRUE),
        total_nonmissing = sum(!is.na(x))
      )
    }) %>% bind_rows()
  }) %>% bind_rows()
}) %>% bind_rows()
...
count_summary <- count_results %>%
  group_by(symptom, age) %>%
  summarise(
    Normal = mean(Normal),
    Borderline_Clinical = mean(Borderline_Clinical),
    total = mean(total_nonmissing),
    
    pct_Normal = mean(Normal / total_nonmissing * 100),
    pct_Borderline_Clinical = mean(Borderline_Clinical / total_nonmissing * 100),
    
    .groups = "drop"
  ) %>%
  mutate(
    Normal_pct = paste0(round(Normal, 1), " (", round(pct_Normal, 1), "%)"),
    Borderline_Clinical_pct = paste0(round(Borderline_Clinical, 1), " (", round(pct_Borderline_Clinical, 1), "%)")
  )

#Plot of odds ratio 

logistic_results_raw <- bind_rows(logistic_results_sqrt)
...
results_list <- list(
  ...
  "Logistic_Results" = logistic_results_sqrt,
  "Stratified_Summary" = summary_all_stratified_sex_bmi_age
)

#write_xlsx(results_list, path = "all_model_results_with_imputed_scores_continueage.xlsx")
